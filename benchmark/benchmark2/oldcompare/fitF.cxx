...
ClassImp(fitF)
   ...
   p21("p21","p21",this,_p21)
 {
    ...
    cout<<"sss "<<pnpaths<<" knri"<<pknri<<endl;
 }
 ...
 Double_t fitF::corefcn(Int_t ndecay,Int_t* decaymap,Int_t* nneu, Double_t* b1n,Double_t* b2n,Double_t* lamda,Double_t N0,Double_t t) const{
     ...
     for (int i=0;i<ndecay-1;i++){
         Double_t corrproductionyield = 1;

         if (nneu[i]==0){
             factor1=factor1 * corrproductionyield*(1-b1n[decaymap[i]-1]-b2n[decaymap[i]-1])*lamda[decaymap[i]-1];
         }else if (nneu[i]==1){
             factor1=factor1 * corrproductionyield* b1n[decaymap[i]-1]*lamda[decaymap[i]-1];
         }else{
             factor1=factor1 * corrproductionyield* b2n[decaymap[i]-1]*lamda[decaymap[i]-1];
         }
     }
     Double_t factor2=0;
 ...
 Double_t fitF::pfcn_decay(Double_t *x, Double_t *par) const{
     Double_t* pn=&par[knri];
     Double_t* lamda=par;
     ...
     for (Int_t i=0;i<pnpaths;i++){
         returnval+=lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0]);
     }
     return returnval;
 }

 //! Function for decay with 1 neutron emission
 Double_t fitF::pfcn_1ndecay(Double_t *x, Double_t *par) const{
     ...
     for (Int_t i=0;i<pnpaths;i++){
         //! random coinc of beta decay of daugter
         returnval+=lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*randcoinf1n;
         //! decay with 1 neutron of daugter nuclei
         returnval+=neueff*pn[pdecaymap[i][pndecay[i]-1]]*lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*(1-randcoinf1n-randcoinfgt0n);

         //! decay with 1 neutron of daugter from p2n
         returnval+=2*(neueff*(1-neueff))*p2n[pdecaymap[i][pndecay[i]-1]]*lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*(1-randcoinf1n-randcoinfgt0n);
         //! decay with 2 neutron of parent (not random 1 neutron)
         returnval-=neueff*neueff*p2n[pdecaymap[i][pndecay[i]-1]]*lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*randcoinf1n;
     }
 ...
 Double_t fitF ::pfcn_2ndecay(Double_t *x, Double_t *par) const{
     Double_t* pn=&par[knri];
     Double_t* lamda=par;
     Double_t* p2n=&par[knri*2];
     Double_t* production_yield=&par[knri*3];
     ...
     for (Int_t i=0;i<pnpaths;i++){
         ...
         returnval+=neueff*neueff*p2n[pdecaymap[i][pndecay[i]-1]]*lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*(1-randcoinf2n-randcoinfgt0n);
         //! random coinc of beta decay of daugter
         returnval+=lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*randcoinf2n;
         //! random 1n decay of daugter
         returnval+=neueff*pn[pdecaymap[i][pndecay[i]-1]]*lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*(randcoinf1n*(1-randcoinfgt0n)-randcoinf2n);

         //! decay with 1 neutron from P2n of daugter - randomly correlated
         returnval+=2*(neueff*(1-neueff))*p2n[pdecaymap[i][pndecay[i]-1]]*lamda[pdecaymap[i][pndecay[i]-1]]*pcorefcn(pndecay[i],pdecaymap[i],pnneu[i],production_yield,pn,p2n,lamda,N0,x[0])*(randcoinf1n*(1-randcoinfgt0n)-randcoinf2n);
     }
     return returnval;
 }
 ...
 Double_t fitF::pcorefcn(Int_t pndecay,Int_t*  ipdecaymap,Int_t*  ipnneu,Double_t* production_yield, Double_t* b1n,Double_t* b2n,Double_t* lamda,Double_t N0,Double_t t) const{
     Double_t fcnret=0;

     Double_t factor1=1.;
     //only parrent decay p2n
     for (int i=0;i<pndecay-1;i++){
         Double_t corrproductionyield = production_yield[ipdecaymap[i+1]];
         if (flag_sum_isomer_ratio){
             for (Int_t j=0;j<nisomers;j++){
                 if (ipdecaymap[i+1] == groundstate[j]){
                     corrproductionyield = 1-production_yield[isomerstate[j]];
                 }
             }
         }
         if (ipnneu[i]==0){
             factor1=factor1 * corrproductionyield*(1-b1n[ipdecaymap[i]]-b2n[ipdecaymap[i]])*lamda[ipdecaymap[i]];//branching here!
         }else if (ipnneu[i]==1){
             factor1=factor1 * corrproductionyield*b1n[ipdecaymap[i]]*lamda[ipdecaymap[i]];
         }else{
             factor1=factor1 * corrproductionyield*b2n[ipdecaymap[i]]*lamda[ipdecaymap[i]];
         }

     }

     Double_t factor2=0;
     for (int i=0;i<pndecay;i++){
         Double_t factor2i=exp(-lamda[ipdecaymap[i]]*t);

         Double_t factor2ij=1;
         for (int j=0;j<pndecay;j++)
             if (j!=i) factor2ij=factor2ij*(lamda[ipdecaymap[j]]-lamda[ipdecaymap[i]]);
         factor2=factor2+factor2i/factor2ij;
     }
     fcnret=factor1*N0*factor2;
     return fcnret;
