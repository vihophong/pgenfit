...
fitF_T12::fitF_T12(const char *name, const char *title,
           RooAbsReal& _x,
           RooAbsReal *_pp[]) :
    RooAbsPdf(name,title),
    x("x","x",this,_x)
{
    ...
    pathfile.close();
    //std::cout<<nri<<std::endl;
    for (Int_t i=0;i<nri*4+2;i++){
        p[i]=new RooRealProxy(Form("p%i",i),Form("p%i",i),this,*_pp[i]);
    }
}


fitF_T12::fitF_T12(const fitF_T12& other, const char* name) :
    RooAbsPdf(other,name),
    x("x",this,other.x)
{
    initPath();
    for (Int_t i=0;i<fpath->nri*4+2;i++)
        p[i]=new RooRealProxy(Form("p%i",i),this,*other.p[i]);
}

void fitF_T12::initPath()
{
    fpath=new path;
    std::ifstream pathfile("path.txt");
    pathfile>>fpath->nri;
...
//#ifndef EVAL_FAST
Double_t fitF_T12::evaluate() const
{
    //TStopwatch stopwatch;
    //stopwatch.Start();
    Double_t l[fpath->nri];
    ...
    Double_t fdaugters[fpath->npaths];
    calculateDecay(fdecayall,fparent, fdaugters, l,e,p1n,p2n,py,N0,be);
//    std::cout<<"AAA"<<fdecayall<<std::endl;
    return fdecayall;
}
//#endif
